...
if ($startRaw && $endRaw) {
    try {
        ...
        $days    = max(1, (int)$startDt->diff($endDt)->days);
    } catch (Exception $e) {
        $days = null;
    }
}
...
if ($cameraId) {
    if ($USE_PDO) {
        $st = $pdo->prepare("SELECT id, name, brand, type, daily_price 
                             FROM cameras WHERE id = :id LIMIT 1");
        $st->execute([':id' => $cameraId]);
        $camera = $st->fetch(PDO::FETCH_ASSOC);
    } else {
        $st = $conn->prepare("SELECT id, name, brand, type, daily_price 
...
<body>
    ...
    <div class="container">
        ...
        <div class="terms-container">
            ...
            <label for="terms" class="terms-text">I agree to the <a href="#" class="terms-link">terms and conditions</a></label>
        </div>
