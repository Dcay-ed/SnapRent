...
$roleUpper  = strtoupper((string)$userRole);
...
/* ================== NOTIFIKASI DARI rentals ==================
   Kita samakan konsep dengan notification.php:
   - Session: $_SESSION['uid'] = accounts.id
   - Relasi: accounts.id -> accounts.customer_id
             -> customers.customer_id -> rentals.customer_id
   - Notif:
       * Order/booking yang sudah dikonfirmasi (status 'confirmed')
...
if ($isLoggedIn && $pdoAvailable && $userId && in_array($roleUpper, ['CUSTOMER', 'COSTUMER'], true)) {

  // Base filter: akun ini (accounts.id) → customers → rentals
  $sqlBase = "
    FROM rentals rn
    JOIN customers c
  ...
  // Notif order berhasil (confirmed)
  $stmt = $pdo->prepare("
    SELECT rn.id,
           rn.total_price,
           rn.start_date,
  ...
  // Notif deadline pengembalian: end_date <= hari ini (tetap pakai base relasi yang sama)
  $today = date('Y-m-d');
  $stmt2 = $pdo->prepare("
    SELECT rn.id,
           rn.total_price,
